use crate::widgets::{
    CameraPaintableClient, NotificationKind, PortalPage, PortalPageExt, PortalPageImpl,
};
use ashpd::{
    desktop::{
        screencast::{CursorMode, ScreenCastProxy, SourceType},
        SessionProxy,
    },
    enumflags2::BitFlags,
    zbus,
...
mod imp {
    ...
    impl ObjectSubclass for ScreenCastPage {
        ...
        type Type = super::ScreenCastClientPage;
...
// a subclass
glib::wrapper! {
    pub struct ScreenCastClientPage(ObjectSubclass<imp::ScreenCastPage>) @extends gtk::Widget, adw::Bin, PortalPage;
}

impl ScreenCastClientPage {
    ...
    async fn start_session(&self) {
        ...
        // TODO remove the screencast, but how
        self.send_notification(
            "Screen cast session started successfully",
            NotificationKind::Success,
        );
        // when I catch two screen , the len of vec become two, and the widget appends two.
        // It is reall nonsence
        let paintable = CameraPaintableClient::new();
        ...
        if let Some(mut child) = self_.streams_carousel.first_child() {
            loop {
                ...
                let paintable = picture
                    .paintable()
                    .unwrap()
                    .downcast::<CameraPaintableClient>()
                    .unwrap();
